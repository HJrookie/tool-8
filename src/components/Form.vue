<template>
  <el-row type="flex" justify="center" style="margin-bottom: 20px" v-if="vis">
    ...
    <el-tag type="info" class="gua" @click="linkTo(guaMap[`${result.m}-${result.b}`])"
      >{{ guaMap[`${result.m}-${result.b}`] }}卦</el-tag
    >
  ...
  <el-row type="flex" justify="center" style="margin-bottom: 20px" v-if="vis">
    ...
    <el-tag type="info" class="gua" @click="linkTo(guaMap[`${bianResult.m}-${bianResult.b}`])"
      >{{ guaMap[`${bianResult.m}-${bianResult.b}`] }}卦</el-tag
    >
    <el-tag type="success">{{ result.t }}爻动</el-tag>
  </el-row>
...
const rules = reactive({
  top: [{ required: true, message: "请输入一个三位数字", trigger: "blur" }],
  mid: [{ required: true, message: "请输入一个三位数字", trigger: "blur" }],
  bottom: [{ required: true, message: "请输入一个三位数字", trigger: "blur" }],
});
